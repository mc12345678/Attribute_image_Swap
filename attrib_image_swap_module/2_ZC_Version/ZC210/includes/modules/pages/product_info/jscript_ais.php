...
// This version needed for ZC 2.1.0 because of breaking code incorporated
//   by: https://github.com/zencart/zencart/pull/6652
//
?>
...
function getattribimage(attribfield, width, height, products_options_values_id, products_id) {
    if (typeof zcJS == "undefined" || !zcJS) {
        ...
        url = "ajax.php?act=attribProdInfo&method=swapImage<?php echo (!empty(zen_get_all_get_params(array('action')))) ? '&' . preg_replace("/&$/","",zen_get_all_get_params(array('action'))) : ''; ?>";
        ...
        var option = { url : "ajax.php?act=attribProdInfo&method=swapImage<?php echo (!empty($_GET['main_page']) ? '&main_page=' . $_GET['main_page'] : '') . ((!empty(zen_get_all_get_params(array('action')))) ? '&' . preg_replace("/&$/","",zen_get_all_get_params(array('action'))) : ''); ?>",
                       data : jsonData,
                       timeout : 30000
                     };
